...
- 教师端必须提供“项目想法提报”入口：教师可简单写下项目想法，系统按事理图谱推理给出年龄层、插入阶段、知识链、生成建议和风险提示，经教师确认后生成 `draft` ProjectPackage 草案并加入审批队列。
...
- 当前草案队列入口只展示已生成草案；下一阶段必须补齐教师从“一句话想法”进入草案队列的 UI 闭环，不允许只让教师手动找 API。
...
- “项目想法提报”流程必须分为两步：先展示事理图谱推理与生成预览，再由教师点击确认生成草案；确认后只进入 `draft` 草案队列，不得直接进入 `ProjectPackage in_review`、`approved` 或 `active`。
- 教师确认可以记录为 `teacher_confirmation` 审核记录；系统校验和教研审核仍要显式留痕，`ready_for_project_package_review` 不是自动入库许可。
- UI 必须明确展示风险边界：L1 不推荐竞赛路径、AI 辅助比例限制、防认知卸载、必备项目包文件和七维评价缺口。

## 完整开发原则

- 开发不能走“最小路径”或只做临时可跑的窄实现。每个阶段都必须交付完整垂直切片，避免后续靠补丁把同一能力补完整。
- “完整”指该阶段承诺的能力从输入、校验、业务服务、API、数据结构、审核边界、测试、文档到 Git 推送都闭环。只要当前阶段会暴露给主系统或后续流程依赖，就必须一次性补齐对应链路。
- 可以分阶段，但每个阶段必须有清晰边界和独立可用结果。允许把“项目草案生成”“审核入库”“服务器部署”等大能力拆到下一阶段，但当前阶段不能留下会导致接口不可用、测试不可运行、审核绕过、数据不可追溯的缺口。
- 禁止使用“先硬编码/先占位/后面再补”作为默认策略。若确实需要临时兼容，必须在文档中写清楚原因、风险、退出条件和下一步提交范围。
- 主系统回接不以“最少文件通过测试”为完成标准，而以“不破坏现有机制、覆盖已知边界、能被教师端/后端后续流程直接接入”为完成标准。
- 事理图谱相关能力至少要同时考虑：年龄阶段约束、知识链、插入位、项目插件审核、AI 防认知卸载、RAG/知识库后续衔接、可回滚和可审计。
- 每次提交前检查是否存在已知遗漏项。如果遗漏项属于当前阶段承诺范围，必须当场补齐；如果属于后续阶段，必须在设计方案或实施方案中明确列为下一阶段任务。
...
- 原型也必须完整覆盖该阶段承诺的输入、输出、边界和测试，不允许只写 happy path。
...
- 后续主系统开发必须按完整垂直切片推进，不允许只补一个 service 或一个 route 就结束；凡是当前阶段需要被调用的能力，都要同步补 schema、service、API、测试、文档和审核边界。
- 下一阶段主系统 UI 回接目标：在教师端 `/teacher/project-package-review` 增加“提报新项目想法”入口，支持输入想法、年龄/层级、课时、硬件、目标用途和教师约束；调用事理图谱定位/草案生成 API 展示推理结果，教师确认后持久化草案并返回草案队列。
- 本阶段主系统落地文件：
...
- 小步提交不等于功能半成品。提交可以小，但每个提交必须让所在阶段向完整闭环推进，不能制造需要马上补救的已知破口。
- 推荐提交粒度：
...
- `域名接入执行清单.md`：`.com` 域名注册、NS 委派、`vsle` A 记录和公网验收的外部执行清单。
